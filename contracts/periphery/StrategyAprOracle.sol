...
contract StrategyAprOracle is AprOracleBase {
    ...
    function aprAfterDebtChange(address _strategy, int256 _delta) external view override returns (uint256) {
        ...
        uint256 assets = vault.expectedLiquidity() + uint256(_delta);
        uint256 baseInterestRate_ = vault.baseInterestRate();
        uint256 nativeYield = (baseInterestRate_ * vault.totalBorrowed() + vault.quotaRevenue() * RAY)
            * (PERCENTAGE_FACTOR - vault.withdrawFee()) / PERCENTAGE_FACTOR / assets;

        // Step 2: Get exchange rate for the asset
        uint256 exchangeRate;
        if (strategy.asset() == WETH) {
            exchangeRate = 1e18; // 1 WETH = 1e18 (1 ETH)
        ...
        uint256 gearInAssetPrice = ICurvePool(GEAR_ETH_CURVE_POOL).price_oracle() * exchangeRate;
        ...
        uint256 supply = staking.totalSupply() + vault.convertToShares(uint256(_delta));
        ...
        uint256 rewardYield =
            (PERCENTAGE_FACTOR * rewardInAssetPrice * SECONDS_PER_YEAR) / (supply * uint256(farmDuration));
